...
def update_user():
    """Реализация изменения пользователя в таблице
     Состоит из двух частей: update_user и edit_row
     Это вторая функция этой пары
     Данная функция нужна для применения изменений в таблице и отображения 
     измененной таблицы пользователю
     """
...
def edit_row(id):
    """Первая функция для изменения данных в таблице
    Необходима для отображения формы для изменении имени и фамилии,
    а также отображает кнопку подтвердить изменения
    """
    users = read_json('users.json')
    data = {
...
# Можно запускать из консоли введя хост и порт или по умолчанию
